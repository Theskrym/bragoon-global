...
def update_html_from_excel(excel_file, output_html):
    ...
    env = Environment(loader=FileSystemLoader('web-scrapper/templates'))
...
def update_static_files(output_path):
    ...
    static_css_path = os.path.join('web-scrapper/static', 'style.css')
    output_css_path = os.path.join(output_path, 'style.css')
    shutil.copy(static_css_path, output_css_path)

    static_js_path = os.path.join('web-scrapper/static', 'scripts.js')
...
def executar_comandos_git():
    ...
    output_dir = r'C:\Users\enzo.machado\Desktop\bragoon\web-scrapper\output'
...
if __name__ == "__main__":
    output_path = os.path.join('web-scrapper', 'output')
    if not os.path.exists(output_path):
        os.makedirs(output_path)

    update_html_from_excel('web-scrapper/output/produtos.xlsx', os.path.join(output_path, 'index.html'))
